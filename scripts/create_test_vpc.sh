#!/usr/bin/env bash
aws cloudformation create-stack --stack-name "test-arole-lansweeper" \
...
aws cloudformation wait stack-create-complete --stack-name "test-arole-lansweeper"

SUBNET_ID="$(aws cloudformation describe-stacks --stack-name "test-arole-lansweeper" --output text | grep 'SubnetPublic0' | awk '{print $9}')"
VPC_ID="$(aws cloudformation describe-stacks --stack-name "test-arole-lansweeper" --output text | grep 'VPCID' | awk '{print $6}')"
